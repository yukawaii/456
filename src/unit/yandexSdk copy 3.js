...
export const loadYandexHighScore = (storeInstance) => {  const platform = getPlatform();    // 1. Быстро из localStorage (мгновенно)
  ...
  } catch(e) {}  // Для ОК — загружаем без проверки инициализации
  if (platform === 'ok') {    if (typeof vkBridge !== 'undefined') {      vkBridge.send('VKWebAppStorageGet', { keys: [CLOUD_STORAGE_KEY] })        .then((data) => {          let cloudScore = 0;          if (data.keys && data.keys[0] && data.keys[0].value) {            cloudScore = parseInt(data.keys[0].value, 10) || 0;          }
          if (cloudScore > 0) {            storeInstance.dispatch(actions.max(cloudScore));            localStorage.setItem('tetris_high_score', cloudScore);
            console.log(`☁️ Загружен рекорд из VK Storage (${platform}):`, cloudScore);          }         // ===== ДОБАВИТЬ ЗАГРУЗКУ ИЗ CLOUDFLARE =====
          if (window.vkUserId) {
            loadCloudScore(window.vkUserId).then(cloudflareScore => {              if (cloudflareScore > 0) {                const currentMax = storeInstance.getState().get('max') || 0;
                if (cloudflareScore > currentMax) {                  storeInstance.dispatch(actions.max(cloudflareScore));                  localStorage.setItem('tetris_high_score', cloudflareScore);
                  console.log(`☁️ Cloudflare: загружен рекорд ${cloudflareScore}`);                }              }            });          }
          // =======================================
        })        .catch(err => console.error('Ошибка загрузки из VK Storage:', err));    }    return;
  }  // Для VK — проверяем инициализацю
  if (platform !== 'vk') return;  if (!vkInitialized || !vkUserId) {    console.warn('[loadYandexHighScore] VK не инициализирован');    return;  }  
  if (typeof vkBridge !== 'undefined') {    vkBridge.send('VKWebAppStorageGet', { keys: [CLOUD_STORAGE_KEY] })      .then((data) => {        let cloudScore = 0;        if (data.keys && data.keys[0] && data.keys[0].value) {
          cloudScore = parseInt(data.keys[0].value, 10) || 0;        }        if (cloudScore > 0) {          storeInstance.dispatch(actions.max(cloudScore));          localStorage.setItem('tetris_high_score', cloudScore);          console.log('☁️ Загружен рекорд из VK Storage:', cloudScore);          
          // Отправляем в таблицу лидеров
          if (vkInitialized && vkUserId && vkUserToken) {            vkBridge.send('VKWebAppCallAPIMethod', {
              method: 'secure.addAppEvent',              request_id: 'syncScore_' + Date.now(),              params: {                client_secret: ACCESS_TOKEN,                user_id: vkUserId,                activity_id: 2,                value: cloudScore,                v: '5.131',
          ...
          }        }      // ===== ДОБАВИТЬ ЗАГРУЗКУ ИЗ CLOUDFLARE =====
        if (window.vkUserId) {
          loadCloudScore(window.vkUserId).then(cloudflareScore => {
            if (cloudflareScore > 0) {
              const currentMax = storeInstance.getState().get('max') || 0;
              if (cloudflareScore > currentMax) {
                storeInstance.dispatch(actions.max(cloudflareScore));
                localStorage.setItem('tetris_high_score', cloudflareScore);
                console.log(`☁️ Cloudflare: загружен рекорд ${cloudflareScore}`);
              }
            }
          });
...
// ===== СОХРАНЕНИЕ РЕКОРДА =====
export const saveYandexScore = (scoreValue) => {  const currentScore = parseInt(scoreValue, 10) || 0;
  if (currentScore <= 0) return;  if (!vkInitialized || !vkUserId || !vkUserToken) {    console.warn('[saveYandexScore] VK не инициализирован или нет токена');
    return;  }  // 1. Проверяем локальный рекорд
  ...
  // ===== ДОБАВИТЬ СОХРАНЕНИЕ В CLOUDFLARE (ДУБЛИРОВАНИЕ) =====
if (window.vkUserId && currentScore > 0) {
  saveCloudScore(window.vkUserId, currentScore);
}
// =======================================================
  // 3. Сохраняем в VK Storage (облако для синхронизации)
  if (typeof vkBridge !== 'undefined') {    vkBridge.send('VKWebAppStorageGet', { keys: [CLOUD_STORAGE_KEY] })
      .then((data) => {        let cloudScore = 0;        if (data.keys && data.keys[0] && data.keys[0].value) {
          cloudScore = parseInt(data.keys[0].value, 10) || 0;
        }        if (currentScore > cloudScore) {          vkBridge.send('VKWebAppStorageSet', {            key: CLOUD_STORAGE_KEY,            value: String(currentScore)
          })          .then(() => console.log(`☁️ Рекорд ${currentScore} сохранён в VK Storage! (было ${cloudScore})`))
          .catch(err => console.error('❌ Ошибка сохранения в VK Storage:', err));
        } else {          console.log(`☁️ Рекорд в облаке не побит: ${currentScore} <= ${cloudScore}`);
        }      })      .catch(err => console.error('❌ Ошибка получения рекорда из VK Storage:', err));  }
  // 4. ТОЛЬКО ДЛЯ ВК: обновляем таблицу лидеров
const platform = getPlatform(); if (platform === 'vk' && vkInitialized && vkUserId && vkUserToken) {  // Сначала получаем текущий рекорд из таблицы лидеров
  vkBridge.send('VKWebAppCallAPIMethod', {    method: 'apps.getScore',    request_id: 'checkScore_' + Date.now(),    params: {      user_id: vkUserId,      v: '5.131',      access_token: vkUserToken    }
    ...
    if (currentScore > currentLeaderboardScore) {      vkBridge.send('VKWebAppCallAPIMethod', {        method: 'secure.addAppEvent',        request_id: 'addScore_' + Date.now(),
        params: {          client_secret: ACCESS_TOKEN,          user_id: vkUserId,          activity_id: 2,          value: currentScore,          v: '5.131',          global: 1,          access_token: vkUserToken        }      })
      .then(() => console.log(`🏆 Рекорд ${currentScore} отправлен в таблицу лидеров ВК! (было ${currentLeaderboardScore})`))
      .catch(err => console.error('❌ Ошибка отправки в таблицу лидеров:', err));    } else {
      console.log(`ℹ️ Рекорд ${currentScore} не превышает табличный ${currentLeaderboardScore}, отправка не требуется`);    }  })
  .catch(err => console.error('❌ Ошибка получения рекорда из таблицы:', err));}
};
